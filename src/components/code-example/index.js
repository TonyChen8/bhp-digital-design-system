...
export default ({ contents, codeType }) => {
  const source = codeType == 'react' && contents.react ? contents.react.source : contents[codeType];

  const ReactExample = contents.react ? contents.react.fn : null;
  return (
    <div className={s.exampleContainer}>
      {contents.title &&
        <h4 className='h4-light'>{contents.title}</h4>
      }
      ...
      <div className={s.exampleDisplay}>
        ...
        {codeType == 'react'
          ? ReactExample ? <div className={s.exampleHtml}><ReactExample /></div> : ''
          : <div
            className={s.exampleHtml}
            dangerouslySetInnerHTML={{ __html: `<style>${contents.css || ''}</style>${contents.actualHtml ? contents.actualHtml : contents.html}` }}
          />
        }
