...
class Template extends React.Component {
  ...
  render() {
    ...
    const filteredCodeTypes = allCodeTypes.filter(({ name }) => _.some(examples, example => {
      return !!example[name];
    }));
    ...
    return (
      <div className={s.uiComponentContainer}>
        <div className='container'>
          ...
          <div
            dangerouslySetInnerHTML={{ __html: contents.fields.preHtml }}
          />
          ...
          {examples.filter(contents => !!contents[codeType]).map((contents, index) =>
            <div
              key={index}
              className={s.codeExampleContainer}
            >
              {(hideCodeTypePicker || index > 0) &&
                <hr />
              }
          ...
          <div
            dangerouslySetInnerHTML={{ __html: contents.fields.postHtml }}
          />
        </div>
      </div>
    );
...
export default ({ contents, reactExamples, hideCodeTypePicker }) => {
  return (
    <div>
      ...
      <Template
        contents={contents}
        hideCodeTypePicker={hideCodeTypePicker}
      />
    </div>
  );
};
